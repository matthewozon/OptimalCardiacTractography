...
void C_test::testSpline(void)
{
    ...
    delete curveSpline;//->~CtlCtlStruct();
    delete dphiSpline;//->~CtlCtlStruct();
    delete ddphiSpline;//->~CtlCtlStruct();
...
void C_test::testComparisonTimeAndParamReal(void)
{
    ...
    if(!spline->computeFiberSpline(FIBERS, &m_ctlCtlStructSpline, 5, CURVE))
    {
        fclose(f);
        delete tool;//->~C_parametric_comparison();
        delete LOAD;//->~C_load_fiber();
        delete FIBERS;//->~CtlCtlStruct();
...
//    for(unsigned long i=0 ; i<N ; i++)
//    {
//        FIBER->elts[3*i+2] = -FIBER->elts[3*i+2];
//    }
    tool->getTimeAndParam(FIBER, &r0, &z0, t);
    printf("%f, %f", r0, z0);
    for(unsigned long i=0 ; i<N ; i++)
...
//        return;
//    }
//    CtlStruct* FIBER = &(m_ctlCtlStructSpline->fibers[2]); //2 5 13 15 16 25 26!
...
void C_test::testHisto(void)
{
    ...
    delete tool;//->~C_measure_spline_synthetic();
    delete LOAD;//->~C_load_fiber();
    return;
}
