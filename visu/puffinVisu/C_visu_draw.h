...
#define MIN_PHYSICAL_LENGTH 1 //0.1
...
//    private:
        ...
        #ifdef ELDEST
            void drawWithOutSplineVtk(CtlCtlStruct* m_ctlCtlStruct, long nbSubDiv, float R, float G, float B, double radius);
        #else
            void drawWithOutSplineVtk(CtlCtlStruct* m_ctlCtlStruct, long nbSubDiv, float R, float G, float B, double radius);
        #endif
        ...
        vtkSmartPointer< vtkActor > drawPOLYDATA( vtkSmartPointer< vtkPolyData > polyDataV);

        #ifdef LATER
            vector< vtkSmartPointer< vtkActor > > drawEdges(SaveEdges* E, unsigned long nbEdges);
            vtkSmartPointer< vtkActor > drawEdge(SaveEdges* E);
        #endif

        double fibLength(CtlStruct* fibers);

        //void CreatePolydata(vtkSmartPointer<vtkPoints> points, vtkSmartPointer<vtkCellArray> lines, vtkSmartPointer<vtkPolyData> polydata);
};
