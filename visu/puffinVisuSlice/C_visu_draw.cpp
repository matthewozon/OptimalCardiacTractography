...
vector< vtkSmartPointer< vtkActor > > C_visu_draw::drawTensors(TensorData* T, unsigned long nbTensor)
{
    ...
    {
        if( sqrt( 0.5*(SQR(T[i].eigenValue[0] - T[i].eigenValue[1]) + SQR(T[i].eigenValue[1] - T[i].eigenValue[2]) + SQR(T[i].eigenValue[2] - T[i].eigenValue[0]))/(SQR(T[i].eigenValue[0]) + SQR(T[i].eigenValue[1]) + SQR(T[i].eigenValue[2])) ) >0.227 )
        {
...
vector< vtkSmartPointer< vtkActor > > C_visu_draw::drawFibers(CtlCtlStruct* Fib, unsigned long nbSubDiv, double radius)
{
    ...
    {
        if(fibLength(&((*splineStruct)->fibers[i]))>MIN_PHYSICAL_LENGTH && (*splineStruct)->fibers[i].N_element>2)
        {
...
vector< vtkSmartPointer< vtkActor > > C_visu_draw::drawRawFibers(CtlCtlStruct* Fib, double radius)
{
    ...
    for(unsigned long i=0 ; i<Fib->N ; i++)
    {
        if(fibLength(&(Fib->fibers[i]))>MIN_PHYSICAL_LENGTH && Fib->fibers[i].N_element>2)
        {
