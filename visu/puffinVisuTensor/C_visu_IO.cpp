...
bool C_visu_IO::load(string fileName, long kind)
{
    ...
    else if(kind==M_TENSOR_ANA)
    {
        return loadTensorFromAnalyze75(fileName);
    }
...
bool C_visu_IO::loadTensorFromAnalyze75(string fileNameTensors)
{
    ///load analyze file: we assume that tensors are stored as xx xy xz yy yz zz
    C_ReadAnalyze75* toolANA = new C_ReadAnalyze75();
    rawData<double>* rawTensor = toolANA->getRawData(fileNameTensors.data());
    //rawTensor->crop4D(5,rawTensor->DimX/2-1, 0, rawTensor->DimY/2-1, 0, rawTensor->DimZ/2-1, 0, rawTensor->DimT-1);
    delete toolANA;
    if(rawTensor->numDim!=4 || rawTensor->DimT!=6)
    {
        delete rawTensor;
        return false;
    }
    ///count number of tensors with FA above a threshold
    double FA_;
    long count=0;
    for(unsigned long i=0 ; i<rawTensor->DimX ; i++)
    {
        for(unsigned long j=0 ; j<rawTensor->DimY ; j++)
        {
            ...
            {
                //if this is not the null tensor
                if( not((rawTensor->raw4D[i][j][k][0]==0.0) && (rawTensor->raw4D[i][j][k][1]==0.0) && (rawTensor->raw4D[i][j][k][2]==0.0) && (rawTensor->raw4D[i][j][k][3]==0.0) && (rawTensor->raw4D[i][j][k][4]==0.0) && (rawTensor->raw4D[i][j][k][5]==0.0)) )
                {
                    if(k*rawTensor->pixDimZ>Zmin && k*rawTensor->pixDimZ<Zmax)
                    {
//                        cout << rawTensor->raw4D[i][j][k][0] << " " << rawTensor->raw4D[i][j][k][1] << " " << rawTensor->raw4D[i][j][k][2] << endl;
//                        cout << rawTensor->raw4D[i][j][k][1] << " " << rawTensor->raw4D[i][j][k][3] << " " << rawTensor->raw4D[i][j][k][4] << endl;
//                        cout << rawTensor->raw4D[i][j][k][2] << " " << rawTensor->raw4D[i][j][k][4] << " " << rawTensor->raw4D[i][j][k][5] << endl;
                        //compute eigen value
...
//                        cout << "FA\^2 = " << FA_ << endl;
    ...
    data->T = new TensorData[count];
    data->nbTensor = count;
    cout << "there shoud be " << count << " points" << endl;
    TensorData* t;
    ///store tensors with FA above the same threshold
    for(unsigned long i=0 ; i<rawTensor->DimX ; i++)
    {
        for(unsigned long j=0 ; j<rawTensor->DimY ; j++)
        {
            for(unsigned long k=0 ; k<rawTensor->DimZ ; k++)
            {
                ...
                if( not((rawTensor->raw4D[i][j][k][0]==0.0) && (rawTensor->raw4D[i][j][k][1]==0.0) && (rawTensor->raw4D[i][j][k][2]==0.0) && (rawTensor->raw4D[i][j][k][3]==0.0) && (rawTensor->raw4D[i][j][k][4]==0.0) && (rawTensor->raw4D[i][j][k][5]==0.0)) )
                {
                    ...
                    {
                        //compute eigen value
                        C_toolbox_eigen_sym* toolEIG = new C_toolbox_eigen_sym(rawTensor->raw4D[i][j][k], true);
                        FA_ = 0.5*(SQR(toolEIG->d[1]-toolEIG->d[0]) + SQR(toolEIG->d[2]-toolEIG->d[1]) + SQR(toolEIG->d[0]-toolEIG->d[2]))/(SQR(toolEIG->d[0])+SQR(toolEIG->d[1])+SQR(toolEIG->d[2]));
                        if(FA_>SQR(m_FAth))
                        {
                            //count back point
                            ...
                            //store tensor
                            t = &(data->T[count]);
                            t->x = i*rawTensor->pixDimX;
                            t->y = j*rawTensor->pixDimY;
                            t->z = k*rawTensor->pixDimZ;
                            t->eigenValue[0] = toolEIG->d[0]; data->T[count].eigenValue[1] = toolEIG->d[1]; data->T[count].eigenValue[2] = toolEIG->d[2];
                            t->eigenVector1[0] = toolEIG->z[0][0];
                            t->eigenVector1[1] = toolEIG->z[1][0];
                            t->eigenVector1[2] = toolEIG->z[2][0];

                            t->eigenVector2[0] = toolEIG->z[0][1];
                            t->eigenVector2[1] = toolEIG->z[1][1];
                            t->eigenVector2[2] = toolEIG->z[2][1];

                            t->eigenVector3[0] = toolEIG->z[0][2];
                            t->eigenVector3[1] = toolEIG->z[1][2];
                            t->eigenVector3[2] = toolEIG->z[2][2];
